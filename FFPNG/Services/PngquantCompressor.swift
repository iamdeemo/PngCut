...
struct PngquantCompressor: ImageCompressor, Sendable {
    ...
    func compress(source: URL, temporaryDestination: URL, progress: @escaping @Sendable (Double) -> Void) async throws {
        ...
        if result.status == 99 {
            do {
                try FileManager.default.copyItem(at: source, to: temporaryDestination)
            } catch {
                throw CompressionFailure.localExecution("pngquant did not produce a smaller file and the original could not be copied: \(error.localizedDescription)")
            }
