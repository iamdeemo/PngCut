...
# Builds pngcut without Developer ID signing or notarization and packages it in
# a compressed DMG. All paths are relative to this script so the command
...
readonly PROJECT_FILE="${PROJECT_DIRECTORY}/PngCut.xcodeproj"
...
STAGING_DIRECTORY="$(mktemp -d "${PROJECT_DIRECTORY}/.build/pngcut-dmg.XXXXXX")"
TEMPORARY_DMG_DIRECTORY="$(mktemp -d "${DISTRIBUTION_DIRECTORY}/.pngcut-dmg.XXXXXX")"
...
echo "Created DMG without Developer ID signing or notarization: ${OUTPUT_DMG}"
