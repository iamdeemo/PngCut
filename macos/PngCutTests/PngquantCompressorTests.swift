...
final class PngquantCompressorTests: XCTestCase {
    ...
    func testCompressorRunsExpectedCommandAndValidatesOutput() async throws {
        ...
        let outcome = try await PngquantCompressor(resolver: PngquantFixedResolver(url: executable)).compress(
            source: source,
    ...
    func testCompressorReturnsNoChangeWhenPngquantExits99() async throws {
        let directory = try makeDirectory()
    ...
    func testCompressorReturnsNoChangeWhenPngquantExits98WithoutOutput() async throws {
        let directory = try makeDirectory()
        let executable = directory.appendingPathComponent("unchanged-pngquant")
        try makeExecutable(at: executable, script: "#!/bin/sh\nexit 98\n")
        let source = directory.appendingPathComponent("source.png")
